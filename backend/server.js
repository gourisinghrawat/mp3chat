...
// ============ REST API Routes for Scheduled Meetings ============
...
app.get('/api/meetings/user/:email', async (req, res) => {
    try {
...
app.patch('/api/meetings/:meetingId/start', async (req, res) => {
    try {
        const { meetingId } = req.params;
        const meeting = await Meeting.findOneAndUpdate(
            { meetingId },
...
app.patch('/api/meetings/:meetingId/end', async (req, res) => {
    try {
        const { meetingId } = req.params;
        const meeting = await Meeting.findOneAndUpdate(
            { meetingId },
...
app.post('/api/meetings/:meetingId/history', async (req, res) => {
    try {
        const { meetingId } = req.params;
        ...
        await history.save();
        console.log(`📝 Meeting history saved: ${meetingId}`);
...
io.on('connection', (socket) => {
    ...
    socket.on('joinMeeting', ({ meetingId, displayName }) => {
        ...
        if (meeting.settings.waitingRoomEnabled && meeting.admin !== userName) {
            ...
            // Use default permissions from meeting settings
    ...
    socket.on('admitFromWaitingRoom', ({ meetingId, userName: userToAdmit }) => {
        ...
        // Add to participants with default permissions
        const defaultPerms = meeting.settings.defaultPermissions || {
            canUnmute: true,
            canVideo: true,
            canScreenShare: false
    ...
    socket.on('applyPermissionsToAll', ({ meetingId, permissions }) => {
        ...
        // Apply to all participants except admin and co-hosts
        meeting.participants.forEach(participant => {
            if (!participant.isAdmin && !participant.isCoHost) {
                ...
                // Notify each participant of their updated permissions
